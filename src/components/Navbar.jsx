    ...
    return (
        <>
            ...
            <nav className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 border-b ${
                ...
                <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 md:px-8 lg:px-12 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-2 md:gap-2.5 no-underline group shrink-0">
                        <div className="overflow-hidden rounded-lg transition-transform duration-300 group-hover:scale-110">
                            <img src={restron} alt="Logo" className='w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 object-cover' />
                        </div>
                        <span className="font-['Sora'] font-extrabold text-base sm:text-lg md:text-xl text-[#1A0A00]">
                            Billing<span className="text-[#C8202D]">Mitra</span>
                        </span>
                    </Link>

                    {/* Desktop Navigation */}
                    <ul className="hidden lg:flex gap-4 xl:gap-8 list-none items-center">
                        {navLinks.map((item) => (
                            <li key={item.name}>
                                <a 
                                    href={item.href} 
                                    className="relative no-underline text-gray-600 font-semibold text-sm xl:text-[0.95rem] transition-colors duration-300 hover:text-[#C8202D] after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-[#C8202D] after:transition-all after:duration-300 hover:after:w-full"
                                >
                                    {item.name}
                                </a>
                            </li>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                        <Link 
                            ...
                            className="hidden md:inline-flex items-center border border-gray-300 rounded-xl px-4 xl:px-5 py-2 xl:py-2.5 text-xs xl:text-sm font-semibold text-[#1A0A00] hover:border-[#C8202D] hover:text-[#C8202D] hover:bg-red-50 transition-all duration-300 no-underline"
                        >
                            Log In
                        </Link>
                        <a 
                            href="https://billing.biosoftech.in/register" 
                            className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-[#C8202D] to-[#E52D3D] text-white px-3 py-2 sm:px-4 sm:py-2.5 md:px-5 md:py-3 rounded-xl font-bold text-[10px] sm:text-xs md:text-sm shadow-md hover:shadow-xl hover:-translate-y-1 active:scale-95 transition-all duration-300 no-underline whitespace-nowrap"
                        >
                        ...
                        {/* Mobile Menu Button */}
                        <button
                            className="lg:hidden flex flex-col items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gray-50 border border-gray-100 text-[#1A0A00] cursor-pointer hover:bg-gray-100 transition-colors"
                            onClick={toggleMenu}
                            aria-label="Toggle menu"
                        >
                            <i className={`${isMenuOpen ? 'ri-close-line' : 'ri-menu-3-line'} text-xl sm:text-2xl`}></i>
                        </button>
                    </div>
                </div>
